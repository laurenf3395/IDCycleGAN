...
class idcyclegan_model1(object):
    def __init__(self,
                 image_batch,
                 video_batch,
                 batch_size=16, #L
    ...
    def build_model(self):
        ...
        dim_img = self.crop_size_img * self.crop_size_img * 3 #L
        ...
        self.g_cost_final = self.g_cost_vid + self.g_cost_img + 1000*(self.reg_img + self.reg_vid)
    ...
    def train(self,
        ...
        if log_summary:
            g_cost_final, reg_img, reg_vid, d_cost_final, summary = session.run(
                [self.g_cost_final, self.reg_img, self.reg_vid, self.d_cost_final, self.summary_op])
            summary_writer.add_summary(summary, step)
            print("Time: %g/itr, Step: %d, generator loss: %g (%g + %g), discriminator_loss: %g" % (
                time.time() - start_time, step, g_cost_final, reg_img, reg_vid, d_cost_final))
        ...
        if generate_sample:
            ...
            write_image(images, sample_dir, 'img_%d_gt.jpg' % step, rows=4)#L
            saveGIFBatch(videos_sample, sample_dir, 'vid_%d_gt' % step)
            write_image(images_gen, sample_dir, 'img_%d_gen.jpg' % step, rows=4)#L wasv 5
            saveGIFBatch(video_sample_gen, sample_dir, 'vid_%d_gen' % step)
            write_image(images_rec, sample_dir, 'img_%d_rec.jpg' % step, rows=4)#L
