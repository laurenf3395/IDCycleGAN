...
def convert_image(images, batch_size, col=4, row=4):
...
def sampleBatch(samples, batch_size, col=4, row=4, frames=32):
    frames = [convert_image(samples[:, i, :, :, :], batch_size, col, row) for i in range(frames)]
    return frames


def write_image(batch, sample_dir, name, rows=4, cols = 4):
