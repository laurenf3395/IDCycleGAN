...
def saveGIFBatch_test(batch, directory, name=''):
    """
...
def convert_image(images, batch_size, col=2, row=2):
...
def sampleBatch(samples, batch_size, col=2, row=2, frames=32):
    frames = [convert_image(samples[:, i, :, :, :], batch_size, col, row) for i in range(frames)]
    return frames


def write_image(batch, sample_dir, name, rows=2, cols = 2):
    batch_size, croop_size, _, channels = np.shape(batch)
    batch = np.clip(((batch + 1.0) * 127.5), 0, 255)
    image = np.zeros((croop_size * rows, croop_size * cols, channels), dtype='uint8')
    for i in range(rows):
        for j in range(cols):
            index = i * cols + j
